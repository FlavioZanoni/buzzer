import { sanitizeView } from '@/lib/imageView';
...
// Persist room to database. Never throws; returns false if the write failed
// so callers that must not report a false "saved" (the editor) can say so.
  ...
  for (let catIdx = 0; catIdx < categories.length; catIdx++) {
    ...
    for (let rowIdx = 0; rowIdx < newCat.clues.length; rowIdx++) {
      ...
      clues.push({
        ...
        view: newClue.kind === 'image' ? sanitizeView(newClue.view) : null,
        answerView:
          newClue.answerKind === 'image' ? sanitizeView(newClue.answerView) : null,
        used,
      });
    }
...
export function updateBonus(room, bonus) {
  ...
  room.game.bonus = {
    ...
    view: bonus.kind === 'image' ? sanitizeView(bonus.view) : null,
    answerView: bonus.answerKind === 'image' ? sanitizeView(bonus.answerView) : null,
    used: oldUsed,
  };
}
...
export function judgeAnswer(room, verdict, electedPlayer) {
  ...
  const setReveal = (player) => {
    room.game.reveal = {
      ...
      content: clue.answer || '',
      view: clue.answer ? clue.answerView || null : null,
